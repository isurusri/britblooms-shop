...
export const getProductByHandle = async function (
  ...
  try {
    ...
    // Handle cases where region doesn't exist or product isn't available in region
    if (
      error instanceof Error &&
      error.message.includes("not found when populating the pricing context")
    ) {
      console.warn(
        `Region ${regionId} not found for product ${handle}. Returning null.`
      )
      return null
    }
